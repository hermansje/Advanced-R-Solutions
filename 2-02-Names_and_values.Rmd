...
1. __[Q]{.Q}__: Explain the relationship between `a`, `b`, `c` and `d` in the following code:
...
2. __[Q]{.Q}__: The following code accesses the mean function in multiple ways. Do they all point to the same underlying function object? Verify with `lobstr::obj_addr()`.
...
3. __[Q]{.Q}__: By default, base R data import functions, like `read.csv()`, will automatically convert non-syntactic names to syntactic names. Why might this be problematic? What option allows you to suppress this behaviour?
...
4. __[Q]{.Q}__: What rules does `make.names()` use to convert non-syntactic names into syntactic names?
    
   __[A]{.solved}__: A valid name starts with a letter or a dot (which must not be followed by a number). It also consists of letters, numbers, dots and underscores only (`"_"` are allowed since R version 1.9.0).
   
   Three main mechanisms ensure syntactically valid names (see `?make.names`):
   - Variable names that do not start with a letter or start with a dot followed by a number will be prepended by an `"X"`
...
5. __[Q]{.Q}__: I slightly simplified the rules that govern syntactic names. Why is `.123e1` not a syntactic name? Read `?make.names` for the full details.
...
1. __[Q]{.Q}__: Why is `tracemem(1:10)` not useful?
...
2. __[Q]{.Q}__: Explain why `tracemem()` shows two copies when you run this code. Hint: carefully look at the difference between this code and the code show earlier in the section.
...
3. __[Q]{.Q}__: Sketch out the relationship between the following objects:
...
1. __[Q]{.Q}__: In the following example, why are `object.size(y)` and `obj_size(y)` so radically different? Consult the documentation of `object.size()`.
...
2. __[Q]{.Q}__: Take the following list. Why is its size somewhat misleading?
    ...
    #> 16,928 B
    ```
    __[A]{.solved}__: It is somewhat misleading, because all three functions are built-in to R as part of the base and stats packages and hence always available.
...
1. __[Q]{.Q}__: Explain why the following code doesn't create a circular list.
...
3. __[Q]{.Q}__: What happens if you attempt to use `tracemem()` on an environment?
